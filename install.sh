...
#
# IMPORTANT: this script requires bash. If you see a "syntax error near
# unexpected token" when running it, you likely invoked it with `sh` instead
# of `bash` (e.g. `sudo sh install.sh` or `curl ... | sh`). The guard below
# re-execs under bash automatically when possible, but always prefer:
#   sudo bash install.sh    /    curl ... | sudo bash
#
if [ -z "$BASH_VERSION" ]; then
    if [ -f "$0" ] && command -v bash >/dev/null 2>&1; then
        ...
        echo "Use:   sudo bash install.sh" >&2
        echo "Or:    curl -fsSL <url>/install.sh | sudo bash" >&2
        exit 1
    fi
fi
...
# When piped via `curl | bash`, BASH_SOURCE[0] does not point at a real
# file, so this resolves to empty and we fall through to bootstrap mode.
SCRIPT_DIR=""
