...
#   sudo bash uninstall.sh   /   curl ... | sudo bash
#
if [ -z "$BASH_VERSION" ]; then
    if [ -f "$0" ] && command -v bash >/dev/null 2>&1; then
        ...
        echo "Use:   sudo bash uninstall.sh" >&2
        echo "Or:    curl -fsSL <url>/uninstall.sh | sudo bash" >&2
        exit 1
    fi
fi
#
# Remote usage (fetches the repo into a temp dir, auto-removed afterward):
#   curl -fsSL https://raw.githubusercontent.com/bshnetwork/netbox-installer/main/uninstall.sh | sudo bash
set -e
