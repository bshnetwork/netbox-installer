...
if [ ! -d "$SCRIPT_DIR/lib" ]; then
    echo "Error: could not find 'lib/' next to update.sh (looked in: $SCRIPT_DIR). Run this
    exit 1
fi
...
[ -d "$NETBOX_DIR" ] || die "No existing NetBox install found at $NETBOX_DIR. Use install.s


echo -e "${C_BOLD}=== NetBox Updater ===${C_RESET}"
resolve_netbox_version
msg_info "Target version: $NETBOX_VERSION"
confirm "Proceed with update? NetBox will be briefly unavailable." y || die "Aborted by use
...
su - "$NETBOX_RUN_USER" -c "tar -xzf /tmp/netbox-new.tar.gz -C '$NEW_DIR' --strip-component
